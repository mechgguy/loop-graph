...
// --- Snap-to-node helper functions ---
/**
 * Returns all snap points for a given strand:
 * - existing node positions
 * - midpoints between consecutive nodes on the same strand
 */
function getSnapPoints(rows, strand) {
  const strandRows = sortedByStrand(rows, strand);
  const points = [];
  
  // Add existing nodes
  strandRows.forEach((r, i) => {
    points.push({
      type: 'node',
      id: r.id,
      x: r.x,
      y: r.y,
      z: r.z,
      visualZ: r.strand === 'Return' ? r.z - 5.0 : r.z,  // FIXED: include visualZ for side view
      strand: r.strand,
      index: i
    });
  });

  // Add midpoints between consecutive nodes
  for (let i = 0; i < strandRows.length - 1; i++) {
    const a = strandRows[i];
    const b = strandRows[i + 1];
    ...
    points.push({
      ...
      y: (a.y + b.y) / 2,
      z: midZ,
      visualZ: strand === 'Return' ? midZ - 5.0 : midZ,  // FIXED
      strand: a.strand,
      index: i + 0.5
    });
  }
...
/**
 * Find the nearest snap point within a given threshold (in screen pixels)
 */
function findNearestSnapPoint(mouseX, mouseY, snapPoints, xScale, yScale, xValue, yValue, thresholdPx = 15) {
  let nearest = null;
  let minDist = thresholdPx;
  
  snapPoints.forEach(sp => {
    const sx = xScale(xValue(sp));
    const sy = yScale(yValue(sp));
...
function MiniPlot({ 
  title, mode, rows, selectedIds, onSelect, onSelectMany, 
  onDragNode, onDragMultipleNodes,
  snapMode = false,        // NEW - are we in add-node snap mode?
  snapStrand = 'Carry',    // NEW - which strand to snap to
  onSnapClick = null       // NEW - callback when user clicks a snap point
}) {
  const svgRef = useRef(null);
  const [stretched, setStretched] = useState(false);
  const [selectionBox, setSelectionBox] = useState(null);
  const dragStartPositions = useRef(new Map());
  const [hoveredSnapPoint, setHoveredSnapPoint] = useState(null);  // NEW
  const w = 760;
  const h = 255;
  ...
  const returnRows = returnRowsRaw.map(r => ({ ...r, visualZ: r.z - visualOffset }));
    // NEW: Compute snap points when in snap mode
  const snapPoints = useMemo(() => {
    if (!snapMode) return [];
    return getSnapPoints(rows, snapStrand);
  }, [rows, snapMode, snapStrand]);
  ...
  const line = d3.line()
    ...
    .y((d) => y(yValue(d)))
    .defined(() => true);
  // NEW: Mouse move handler for snap highlighting
  function handleSnapMouseMove(event) {
    if (!snapMode || !svgRef.current) return;
    ...
    setHoveredSnapPoint(nearest);
  }

  // NEW: Click handler for snap mode
  function handleSnapClick(event) {
    if (!snapMode || !onSnapClick || !hoveredSnapPoint) return;
    event.stopPropagation();
    onSnapClick(hoveredSnapPoint);
  }
  ...
  return (
    <section className="plot-card">
      ...
      <svg 
        ref={svgRef} 
        viewBox={`0 0 ${w} ${h}`}
        onMouseMove={snapMode ? handleSnapMouseMove : undefined}
        onClick={snapMode ? handleSnapClick : undefined}
        style={{ cursor: snapMode ? 'crosshair' : 'default' }}
      >
                <rect className="background" width={w} height={h} fill="transparent" 
          onMouseDown={snapMode ? undefined : startBoxSelect} 
          style={{ cursor: snapMode ? 'crosshair' : 'crosshair' }} />
                  {/* NEW: Render snap points when in snap mode */}
         {/* NEW: Render snap points when in snap mode - only show when hovered or near */}
        {snapMode && snapPoints.map((sp, i) => {
          const isHovered = hoveredSnapPoint === sp;
          // Only render if hovered (near cursor), hide distant snap points
          if (!isHovered) return null;
          
          const isNode = sp.type === 'node';
          const isMidpoint = sp.type === 'midpoint';
          
          return (
            <g key={`snap-${sp.type}-${i}`}>
              {/* Outer glow ring */}
              <circle
                cx={x(xValue(sp))}
                cy={y(yValue(sp))}
              ...
              {isMidpoint && (
                <g style={{ cursor: 'pointer' }}>
                  ...
                  />
                  <line 
                    x1={x(xValue(sp)) - 3} y1={y(yValue(sp))} 
                    x2={x(xValue(sp)) + 3} y2={y(yValue(sp))} 
                    stroke="#006666" strokeWidth={1.5}
                  />
                  <line 
                    x1={x(xValue(sp))} y1={y(yValue(sp)) - 3} 
                    x2={x(xValue(sp))} y2={y(yValue(sp)) + 3} 
                    stroke="#006666" strokeWidth={1.5}
                  />
                </g>
              )}
              ...
              >
                {isMidpoint ? `Midpoint` : `Node ${sp.id}`}
              </text>
            </g>
          );
        })}
...
function App() {
  ...
  const [snapMode, setSnapMode] = useState(false);     // NEW
  const [snapStrand, setSnapStrand] = useState('Carry'); // NEW
  ...
  function fitNodesToWireframe() {
    ...
    // NEW: Snap click handler
  // NEW: Snap click handler - inserts at correct strand position
  function handleSnapClick(snapPoint) {
    recordHistory();
    ...
    if (snapPoint.type === 'node') {
      // Clicked on an existing node - insert right after it (like "Insert After")
      newNode = {
        id: newId,
        strand: snapPoint.strand,
        strandValue: snapPoint.strand === 'Return' ? 1 : -1,
        group: snapPoint.strand === 'Return' ? 1 : 0,
        x: snapPoint.x + 0.1,
        y: snapPoint.y + 0.1,
        z: snapPoint.z,
        originalZ: snapPoint.z,
        tags: `Snapped after Node ${snapPoint.id}`,
        fromCsv: false,
        fittedToWireframe: false,
        wireframeZ: null
      };
      
      // Insert right after the snapped node in the full rows array
      setRows(prev => {
        const snapIndex = prev.findIndex(r => r.id === snapPoint.id);
        if (snapIndex >= 0) {
          const newRows = [...prev];
          newRows.splice(snapIndex + 1, 0, newNode);
          return newRows;
        }
        return [...prev, newNode];
      });
      
      setStatus(`Added Node ${newId} after Node ${snapPoint.id}`);
    } else {
      // Midpoint - insert BETWEEN the two surrounding strand nodes
      newNode = {
        id: newId,
        strand: snapPoint.strand,
        strandValue: snapPoint.strand === 'Return' ? 1 : -1,
        ...
        z: snapPoint.z,
        originalZ: snapPoint.z,
        tags: `Inserted at midpoint`,
        fromCsv: false,
        fittedToWireframe: false,
        wireframeZ: null
      };
      
      // Find the two surrounding nodes on this strand and insert between them
      setRows(prev => {
        const strandRows = sortedByStrand(prev, snapPoint.strand);
        
        // Find which pair of nodes this midpoint belongs to
        ...
        for (let i = 0; i < strandRows.length - 1; i++) {
          ...
          const midDist = Math.hypot(midX - snapPoint.x, midY - snapPoint.y);
        ...
        if (insertAfterId !== null) {
          const insertIndex = prev.findIndex(r => r.id === insertAfterId);
          if (insertIndex >= 0) {
            const newRows = [...prev];
            newRows.splice(insertIndex + 1, 0, newNode);
            return newRows;
          }
        }
      ...
      setStatus(`Added Node ${newId} at midpoint`);
    }
    
    // Exit snap mode after adding
    setSnapMode(false);
  ...
  // Add node - now toggles snap mode when nodes exist
  function addNode() {
    recordHistory();
    if (snapMode) {
      // If already in snap mode, exit it (cancel)
      setSnapMode(false);
      setStatus('Snap mode cancelled');
      return;
    }
    
    if (rows.length === 0) {
      // No nodes to snap to - add at origin
      recordHistory();
      const newId = nextNodeIdRef.current++;
      const newNode = {
        id: newId,
      ...
      setStatus(`Added node ${newId} at origin (no existing nodes to snap to)`);
      return;
    }
    
    // Determine which strand to snap to based on last selected
    if (lastSelectedId) {
      const selNode = rows.find(r => r.id === lastSelectedId);
      if (selNode) {
        setSnapStrand(selNode.strand);
      }
    }
    
    setSnapMode(true);
    setStatus('Snap mode ON — Click a yellow node or cyan midpoint to place new node. Click "+ Add Node" again to cancel.');
  }
  ...
  function exportToPng() {
    // 1. Find the 3D canvas element
    const canvas = document.querySelector('canvas'); 
    if (!canvas) {
      setStatus("Error: 3D Canvas not found.");
      return;
    }

    try {
      // 2. Convert canvas to data URL
      // Note: If the image is black, you may need to trigger a re-render 
      // or ensure 'preserveDrawingBuffer: true' is in your Three.js setup.
      const dataUrl = canvas.toDataURL("image/png");
      
      // 3. Create a temporary link to trigger download
      const link = document.createElement('a');
      link.download = `conveyor-3d-view-${new Date().getTime()}.png`;
      link.href = dataUrl;
      link.click();
      
      setStatus("3D View exported as PNG.");
    } catch (err) {
      console.error(err);
      setStatus("Export failed: Canvas security or buffer issue.");
    }
  }
  // BEFORE SUBMIT ENDS
    ...
    <main className="editor">
      ...
      >
        ...
        <button onClick={resetAll}>RESET ALL</button>
          ...
          className="primary" 
          onClick={addNode}
          style={snapMode ? { backgroundColor: '#ff9800', fontWeight: 'bold' } : {}}
        >
          {snapMode ? '🔍 Snap Mode ON (Click to cancel)' : '+ Add Node'}
        </button>
          ...
          <section className="visual-area">
            <div className="left-plots">
              <MiniPlot title="SIDE VIEW" mode="side" rows={rows}
                ...
                snapMode={snapMode} snapStrand={snapStrand} onSnapClick={handleSnapClick} />
