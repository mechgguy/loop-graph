  ...
  return (
    <section className="plot-card">
      ...
      <svg ref={svgRef} viewBox={`0 0 ${w} ${h}`} onClick={(e) => {
        // Only clear if clicking directly on the SVG background, not on a circle
...
function buildConveyor(scene, lineRows, strand, toVec, selectedId) {
    if (!lineRows) {
    // Don't build anything if less than 2 points
    return;
  }
...
useEffect(() => {
  ...
  // Clear old conveyor objects (ALWAYS clear, even if rows is empty)
  while (conveyorGroup.children.length > 0) {
    const obj = conveyorGroup.children[0];
    conveyorGroup.remove(obj);
    if (obj.geometry) obj.geometry.dispose();
  ...
  // If no rows, we're done (conveyor group is already cleared)
  if (rows.length === 0) {
    return;
  }

  // Continue with building conveyors only if we have rows
  function toVec(r) {
    return new THREE.Vector3(r.x, r.y, r.z);
  }
  ...
  if (carryRows.length > 0) {
    buildConveyor(scene, carryRows, 'Carry', toVec, selectedId);
  }
  
  if (returnRows.length > 0) {
    buildConveyor(scene, returnRows, 'Return', toVec, selectedId);
  }
...
function App() {
  ...
  function updateMultipleNodes(nodeUpdates) {
    setRows((prev) =>
      prev.map((r) => {
        if (nodeUpdates.has(r.id)) {
          return {
            ...r,
            ...nodeUpdates.get(r.id)
          };
        }
        return r;
      })
  ...
  function resetWireframeScale() {
    ...
    if (originalRowsRef.current) {
      const originalRows = JSON.parse(JSON.stringify(originalRowsRef.current));
      setRows(originalRows);
      setStatus('Reset: wireframe scale to 1.00x and conveyor nodes to original CSV values.');
    } else {
      setStatus('Wireframe scale reset to 1.00x. No original data to restore.');
    }
  }
  ...
  function deleteNode() {
    ...
    const willDeleteAll = selectedIds.size === rows.length;
    ...
    if (willDeleteAll) {
      setStatus(`Deleted all ${count} nodes - conveyor cleared`);
    } else {
  ...
  return (
    <main className="editor">
      ...
      <div className="toolbar">
        ...
        <button onClick={resetWireframeScale}>RESET ALL</button>
        ...
        <button className="danger" disabled={selectedIds.size === 0} onClick={deleteNode}>
          Delete Node
        </button>
      ...
      <section className="visual-area">
        <div className="left-plots">
          <MiniPlot
            ...
            onDragMultipleNodes={updateMultipleNodes}
          />
        ...
        <View3D
          ...
          onSelect={(row) => setSelected(row, false)}
