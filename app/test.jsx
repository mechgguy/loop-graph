...
const HEIGHTMAP_PATH = '/Highland_valley.png';
    ...
    <section className="plot-card">
      ...
      {mode === 'side' && (
        <div style={{ fontSize: '10px', color: '#666', marginTop: '8px', textAlign: 'center' }}>
          💡 Tip: Toggle between projected X/Z and stretched belt-length/Z views.
        </div>
      )}
    </section>
  );
...
function addTerrainMesh(planeScale, zScale) {
  const effectiveTerrainSize = TERRAIN_SIZE * planeScale;
  const effectiveDisplacementScale = TERRAIN_DISPLACEMENT_SCALE * zScale;

  const geometry = new THREE.PlaneGeometry(effectiveTerrainSize, effectiveTerrainSize, 512, 512);
  ...
  const heightmapTexture = textureLoader.load(HEIGHTMAP_PATH, () => {
...
function createConveyorRibbonGeometry(points, width = 2.4) {
  const vertices = [];
  const indices = [];
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    vertices.push(left.x, left.y, left.z + 0.15);
    vertices.push(right.x, right.y, right.z + 0.15);
...
function addTube(scene, points, color, radius = 0.18) {
...
function addRoller(scene, position, tangent, width, selected = false) {
  ...
  const geometry = new THREE.CylinderGeometry(0.35, 0.35, width + 1.2, 18);
...
function addSupport(scene, position) {
  ...
  const geometry = new THREE.CylinderGeometry(0.12, 0.12, height, 12);
...
function buildConveyor(scene, lineRows, strand, toVec, selectedId) {
  ...
  const beltWidth = strand === 'Carry' ? 2.4 : 2.0;
  ...
  const railOffset = beltWidth / 2 + 0.3;
  const leftRail = [];
  const rightRail = [];
  ...
  for (let i = 0; i < sampled.length; i++) {
    ...
    leftRail.push(sampled[i].clone().add(side).add(new THREE.Vector3(0, 0, 0.4)));
    rightRail.push(sampled[i].clone().sub(side).add(new THREE.Vector3(0, 0, 0.4)));
  }

  addTube(scene, leftRail, color, 0.17);
  addTube(scene, rightRail, color, 0.17);
  ...
  lineRows.forEach((row) => {
    ...
    const geometry = new THREE.SphereGeometry(selected ? 1.2 : 0.8, 24, 24);
...
function View3D({ rows, selectedId, onSelect, planeScale, zScale }) {
  ...
  useEffect(() => {
    ...
    const terrain = addTerrainMesh(planeScale, zScale);
    scene.add(terrain);
  ...
  }, [rows, selectedId, planeScale, zScale]);
...
function App() {
  ...
  useEffect(() => {
    loadHeightmapSampler(HEIGHTMAP_PATH)
      .then((sampler) => {
  ...
  function updatePlaneScale(value) {
    ...
    setStatus(
      `Plane scale set to ${next.toFixed(2)}x. Terrain footprint is ${(TERRAIN_SIZE * next).toFixed(
        0
      )} m × ${(TERRAIN_SIZE * next).toFixed(0)} m. Press FIT again to resnap nodes.`
    );
  }
  ...
  function updateZScale(value) {
    ...
    setStatus(
      `Z scale set to ${next.toFixed(2)}x. Terrain height range is ${(
        TERRAIN_DISPLACEMENT_SCALE * next
      ).toFixed(0)} m. Press FIT again to resnap nodes.`
    );
  }
  ...
  function resetWireframeScale() {
    ...
    setFittedIds(new Set());
    setStatus(
      `Wireframe reset. Terrain footprint is ${TERRAIN_SIZE} m × ${TERRAIN_SIZE} m and height range is ${TERRAIN_DISPLACEMENT_SCALE} m. Press FIT again to resnap nodes.`
    );
  }
  ...
  function fitNodesToWireframe() {
    ...
    const fittedRows = rows.map((row) => {
      ...
      const wireframeZ = heightSampler(
        localX,
        localY,
    ...
    setStatus(
      adjusted.size > 0
        ? `FIT complete: adjusted ${adjusted.size} node${
            adjusted.size === 1 ? '' : 's'
          }. Rule: below terrain → terrain + ${MIN_CLEARANCE_ABOVE_WIREFRAME} m, above terrain → max terrain + ${MAX_CLEARANCE_ABOVE_WIREFRAME} m.`
        : `FIT complete: no nodes needed adjustment. All nodes are within the allowed terrain clearance range.`
    );
  }
  ...
  return (
    <main className="editor">
      ...
      <div className="toolbar">
        ...
        <button onClick={resetWireframeScale}>RESET WIRE</button>
